...
const VideoBackground = ({ movieId, isMoviePage }) => {
    ...
    return (
        <section className={`${isMoviePage ? "w-full h-full" : "w-full h-[55%] md:h-full pt-28 md:pt-48 lg:pt-0"} bg-[#050505]`}>
            <div className={`${isMoviePage ? "w-full h-full relative" : "w-full h-[55%] lg:h-full"}`}>
                <iframe
                    className={`w-full h-full ${isMoviePage ? "absolute inset-0" : "scale-150 md:scale-[2.9] lg:scale-125"}`}
